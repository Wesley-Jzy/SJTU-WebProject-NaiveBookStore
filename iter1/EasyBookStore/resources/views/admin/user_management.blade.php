...
@section('content')
    ...
    <div class="container" align="center">
        <table id="dg" title="用户信息" class="easyui-datagrid" style="width:700px;height:370px"
                url="user_management/get_user" method="get"
                toolbar="#toolbar" pagination="true" idField="id"
                rownumbers="true" fitColumns="true" singleSelect="true">
            <thead>
                <tr>
                    <th field="id" width="50" editor="{type:'validatebox',options:{required:true}}">ID</th>
                    <th field="name" width="50" editor="{type:'validatebox',options:{required:true}}">用户名</th>
        ...
        function removeUser(){
          ...
          if (row){
            $.messager.confirm('确认','真的要删除吗？',function(r){
              if (r){
                $.post('user_management/'+row.id, {_method:"delete"}, function(result){
                  if (result.success){
                    $('#dg').datagrid('reload');// reload the user data
                  } else {
